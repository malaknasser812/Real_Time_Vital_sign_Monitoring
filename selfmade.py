...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.play_pause_ch1.clicked.connect(lambda: self.play_pause())
        self.replay_ch1.clicked.connect(lambda: self.replay())
        self.zoom_in_ch1.clicked.connect(lambda: self.zoomIn())
        self.zoom_out_ch1.clicked.connect(lambda: self.zoomOut())
        self.add_to_rep_ch1.clicked.connect(lambda: self.add_to_rep())

        self.pushButton_6.clicked.connect(lambda: self.play_pause())
        self.pushButton_7.clicked.connect(lambda: self.replay())
        self.pushButton_8.clicked.connect(lambda: self.zoomIn())
        self.pushButton_9.clicked.connect(lambda: self.zoomOut())
        self.pushButton_10.clicked.connect(lambda: self.add_to_rep())
        ...
        self.horizontalSlider_speed_ch1.valueChanged.connect(lambda: self.speed_adjust())
        self.horizontalSlider_2.valueChanged.connect(lambda: self.speed_adjust())

        # SIGNAL SLIDERS

        self.horizontalSlider_ch1.valueChanged.connect(lambda: self.plotHSlide())
        self.verticalSlider_4_ch1.valueChanged.connect(lambda: self.plotVSlide())
        self.horizontalSlider_3.valueChanged.connect(lambda: self.plotHSlide())
        self.verticalSlider_3.valueChanged.connect(lambda: self.plotVSlide())
    ...
    def load(self):
        if self.ch1.isChecked() == True:
            ...
            path1 = self.fname1[0] #store the file path
            data1 = pd.read_csv(path1)
            self.y1 = data1.values[:, 1]
            self.y1max = max(self.y1) # min y axis value
            self.y1min = min(self.y1) # max y axis value
            self.x1 = data1.values[:, 0] # min x axis value
            self.x1max = self.x1[-1] # max x axis value
            self.ch1loaded = True
            self.Play()
        elif self.ch2.isChecked() == True:
